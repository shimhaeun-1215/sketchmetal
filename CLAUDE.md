...
## 아직 끝나지 않은 것
- 견적 문의 메일: `SITE.formEndpoint` 가 비어 있어 접수 완료 화면만 나옴(시안). 폼 서비스 또는 구글 앱스 스크립트 연결 필요
- 카카오맵: `SITE.kakaoMapKey` 가 비어 있어 위치 카드만 보임. 키 발급 + 웹 플랫폼에 배포 주소 등록 필요. 실제 카카오 서버로는 시험하지 못하고 가짜 응답으로만 흐름을 확인함
